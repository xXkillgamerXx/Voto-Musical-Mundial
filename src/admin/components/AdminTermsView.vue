...
import { getAdminTerms, updateAdminTerms } from '../../services/api/adminApi'
...
const loadTerms = async () => {
  ...
  try {
    const payload = await getAdminTerms()
    form.value = {
      es: {
    ...
    errorMessage.value = error?.message || 'No se pudieron cargar los términos.'
  } finally {
...
const saveTerms = async () => {
  ...
  try {
    ...
    const payload = await updateAdminTerms({
      es: {
    ...
    updatedAt.value = payload?.updatedAt || null
    successMessage.value = 'Términos y condiciones guardados.'
  } catch (error) {
    errorMessage.value = error?.message || 'No se pudieron guardar los términos.'
  } finally {
  ...
  <section class="space-y-6">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Edita el contenido legal en español e inglés con texto enriquecido.
            Se muestra en
            <a
    ...
    <article class="rounded-4xl border border-white/10 bg-white/4 p-5 sm:p-6">
      ...
      <template v-else>
        ...
        <form class="mt-5 grid gap-6 xl:grid-cols-[1fr_0.9fr]" @submit.prevent="saveTerms">
          <div class="space-y-4">
            <label class="grid gap-2">
              ...
              <input
                ...
                :placeholder="activeLocale === 'es' ? 'Términos y condiciones' : 'Terms and conditions'"
              />
            </label>
            ...
            <div class="terms-editor grid gap-2">
              <span class="text-[10px] font-black uppercase tracking-widest text-slate-500">
                Contenido (texto enriquecido)
              </span>
              <RichTextEditor
                v-model="form[activeLocale].bodyHtml"
                :placeholder="activeLocale === 'es' ? 'Escribe los términos en español...' : 'Write the terms in English...'"
              />
            </div>
          </div>
...
<style scoped>
.terms-editor :deep(.ql-container.ql-snow),
