...
const shouldShowForm = computed(() => props.showForm || Boolean(editingCategoryId.value))
const layoutClass = computed(() => shouldShowForm.value ? 'xl:grid-cols-[0.45fr_1fr]' : 'xl:grid-cols-1')
...
<template>
  <section class="grid gap-6" :class="layoutClass">
    <article
      v-if="shouldShowForm"
      class="rounded-3xl border border-fuchsia-300/20 bg-[#090b19] p-5 shadow-2xl shadow-fuchsia-950/30 sm:p-6"
    >
      ...
      <form class="mt-5 space-y-4" @submit.prevent="saveCategory">
        <label class="block">
