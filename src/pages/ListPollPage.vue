...
const activeRound = computed(() =>
  poll.value?.status === 'closed'
    ? rounds.value.filter((round) => round.status === 'closed').at(-1) || rounds.value.at(-1) || null
    : rounds.value.find((round) => round.id === poll.value?.activeRoundId)
      || rounds.value.find((round) => round.status === 'live')
      || rounds.value[0]
      || null,
)
const pollEndDate = computed(() => activeRound.value?.endAt?.toDate?.() || poll.value?.endAt?.toDate?.() || null)
const hasEnded = computed(() => Boolean(pollEndDate.value && pollEndDate.value.getTime() <= now.value))
const isVotingOpen = computed(() => poll.value?.status === 'live' && !hasEnded.value)
const isSelectingWinners = computed(() =>
  poll.value?.status === 'selecting_winners' || (poll.value?.status === 'live' && hasEnded.value),
)
...
const winners = computed(() =>
  rankedContestants.value.filter((contestant) =>
    contestant.isWinner || poll.value?.winnerIds?.includes(contestant.artistId),
  ),
)
...
const previousRoundWinners = computed(() =>
  (previousRound.value?.winnerIds || []).map((artistId) => getArtist(artistId)).filter(Boolean),
)
...
const roundIntroRemainingSeconds = computed(() => {
  ...
  return Math.max(Math.ceil((10000 - (now.value - startedAt.getTime())) / 1000), 0)
})

const isRoundIntroVisible = computed(() =>
  isVotingOpen.value && previousRoundWinners.value.length > 0 && roundIntroRemainingSeconds.value > 0,
)
...
const roundSteps = computed(() =>
  rounds.value
    .map((round, index) => {
      ...
      const activeRoundIndex = rounds.value.findIndex((item) => item.id === activeRound.value?.id)
      const isBeforeActiveRound = activeRoundIndex > -1 && index < activeRoundIndex
      ...
      return {
        ...
        winners: (round.winnerIds || []).map((artistId) => getArtist(artistId)).filter(Boolean),
      }
    })
...
const shouldShowUpcomingRound = computed(() =>
  Boolean(rounds.value.length && !rounds.value.some((round) => round.status === 'live')),
)
...
const countdown = computed(() => {
  ...
  const remainingSeconds = Math.max(Math.floor((pollEndDate.value.getTime() - now.value) / 1000), 0)
...
const listenContestants = (pollId) => {
  ...
  const contestantsCollection = activeRound.value
    ? collection(db, 'polls', pollId, 'rounds', activeRound.value.id, 'contestants')
...
const listenRounds = (pollId) => {
  ...
  unsubscribeRounds = onSnapshot(
    query(collection(db, 'polls', pollId, 'rounds'), orderBy('createdAt', 'asc')),
    (roundsSnap) => {
...
const loadPoll = async () => {
  ...
  try {
    ...
    const pollDoc = pollsSnap.docs.find((docSnap) => {
...
const voteFor = async (contestant) => {
  ...
  try {
    ...
    const contestantRef = activeRound.value
      ? doc(db, 'polls', poll.value.id, 'rounds', activeRound.value.id, 'contestants', contestant.artistId)
...
<template>
  <section class="mx-auto max-w-352 px-4 pb-16 sm:px-6">
    ...
    <template v-else>
      <article class="overflow-hidden rounded-4xl border border-violet-300/20 bg-[#080a18] shadow-2xl shadow-fuchsia-950/25">
        <div class="relative min-h-80 bg-linear-to-br from-violet-950 to-fuchsia-950">
          <img
            v-if="poll?.banner"
            :src="poll.banner"
            :alt="poll.title"
          ...
          />
          <div class="absolute inset-0 bg-linear-to-t from-[#080a18] via-[#080a18]/45 to-black/20"></div>
          <div class="absolute inset-x-0 bottom-0 p-5 sm:p-8">
            <p class="text-xs font-black uppercase tracking-[0.3em] text-fuchsia-300">
              Votacion en vivo
            </p>
            <h1 class="mt-3 text-4xl font-black leading-none text-white sm:text-6xl">
              {{ poll?.title }}
            </h1>
      ...
      <div class="mt-6 flex justify-center">
        <div class="grid w-full max-w-2xl grid-cols-4 gap-2 rounded-3xl border border-white/10 bg-white/5 p-4">
          <div
          ...
          >
            ...
            <p class="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ item.label }}</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="mt-5 overflow-x-auto pb-1">
          <div class="flex min-w-max items-start gap-3">
            <div
              v-for="round in roundSteps"
              :key="round.id"
              class="relative flex w-42 flex-col items-center text-center"
            >
              <div class="absolute left-1/2 top-7 h-px w-full bg-white/10"></div>
              <div
              ...
              <p class="mt-1 max-w-36 truncate text-sm font-black text-white">
                {{ round.title }}
              </p>
              <p
                class="mt-1 rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-widest"
                :class="round.isActive ? 'bg-cyan-400/10 text-cyan-200' : round.status === 'closed' ? 'bg-amber-400/10 text-amber-200' : 'bg-white/5 text-slate-500'"
              >
              ...
              <div v-if="round.winners.length" class="mt-3 flex justify-center -space-x-2">
                <span
                  v-for="artist in round.winners.slice(0, 3)"
                  :key="artist.id"
                  class="grid size-10 place-items-center overflow-hidden rounded-full border-2 border-[#080a18] bg-linear-to-br from-amber-300 to-fuchsia-500 text-xs font-black text-white"
                  :title="artist.name"
                >
                  <img
              ...
              <p v-if="round.winners.length" class="mt-2 max-w-36 truncate text-xs font-bold text-amber-100">
                Ganó {{ round.winners.map((artist) => artist.name).join(', ') }}
              </p>
            </div>
            ...
            >
              <div class="grid size-14 place-items-center rounded-full border border-dashed border-white/20 bg-slate-950/50 text-lg font-black text-slate-500">
                +
              </div>
              <p class="mt-3 text-xs font-black uppercase tracking-widest text-slate-500">
                Siguiente
              </p>
      ...
      >
        ...
        <div class="winner-light-secondary pointer-events-none absolute -bottom-24 right-0 size-96 rounded-full bg-fuchsia-400/15 blur-3xl"></div>
        <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(251,191,36,0.18),transparent_34%),radial-gradient(circle_at_85%_30%,rgba(217,70,239,0.14),transparent_28%)]"></div>

        <div class="relative">
          <div class="winner-crown mx-auto grid size-20 place-items-center rounded-4xl border border-amber-300/30 bg-amber-300/10 text-4xl shadow-xl shadow-amber-950/30">
            ♛
          </div>
          ...
          <p class="mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-300 sm:text-base">
      ...
      >
        ...
        <div class="relative">
          ...
          <p class="mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-300 sm:text-base">
            En unos segundos comienza {{ activeRound?.title || 'la siguiente ronda' }}.
          </p>
        </div>
        ...
        <div class="relative mx-auto mt-10 grid max-w-4xl gap-4 md:grid-cols-2">
          ...
          >
            <span class="grid size-12 shrink-0 place-items-center rounded-2xl bg-amber-300/15 text-sm font-black text-amber-100">
              #{{ index + 1 }}
            </span>
            <span class="grid size-20 shrink-0 place-items-center overflow-hidden rounded-3xl bg-linear-to-br from-amber-300 to-fuchsia-500 text-xl font-black text-white">
              <img
                v-if="getArtistImage(winner)"
                :src="getArtistImage(winner)"
                :alt="winner.name"
            ...
            <span class="min-w-0">
              <span class="block truncate text-2xl font-black text-white">{{ winner.name }}</span>
              <span class="text-sm font-bold uppercase text-amber-100">{{ getArtistGroup(winner) || 'Ganador de ronda' }}</span>
            </span>
          </article>
        </div>

        <div class="relative mx-auto mt-8 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-5 py-3 shadow-xl shadow-amber-950/20 backdrop-blur">
      ...
      >
        <div class="waiting-aurora pointer-events-none absolute -left-24 -top-24 size-80 rounded-full bg-fuchsia-500/20 blur-3xl"></div>
        <div class="waiting-aurora-secondary pointer-events-none absolute -bottom-28 right-0 size-96 rounded-full bg-cyan-400/15 blur-3xl"></div>
        <div class="waiting-orbit pointer-events-none absolute left-1/2 top-1/2 size-96 -translate-x-1/2 -translate-y-1/2 rounded-full border border-fuchsia-300/10"></div>
        <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(217,70,239,0.22),transparent_28%),radial-gradient(circle_at_80%_20%,rgba(34,211,238,0.16),transparent_25%)]"></div>
        <div class="pointer-events-none absolute left-0 top-0 h-px w-full animate-pulse bg-linear-to-r from-transparent via-fuchsia-300/70 to-transparent"></div>

        <div class="relative mx-auto max-w-3xl">
          <div class="waiting-symbol relative mx-auto grid size-24 place-items-center rounded-4xl border border-fuchsia-300/25 bg-fuchsia-400/10 text-4xl text-fuchsia-100 shadow-xl shadow-fuchsia-950/30 sm:size-28">
            ✦
          </div>
          <p class="relative mt-8 text-xs font-black uppercase tracking-[0.3em] text-fuchsia-300">
            Conteo en proceso
          </p>
          <h2 class="relative mx-auto mt-4 max-w-2xl text-4xl font-black text-white sm:text-6xl">
            Estamos contando los votos
          </h2>
          <p class="relative mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
            La votación terminó. Estamos revisando los resultados en tiempo real y eligiendo a los ganadores. Espera un momento.
          </p>

          <div class="relative mx-auto mt-9 flex max-w-md items-center justify-center gap-3 rounded-full border border-white/10 bg-white/5 px-5 py-3 shadow-xl shadow-fuchsia-950/20 backdrop-blur">
            <span class="size-2 animate-bounce rounded-full bg-fuchsia-300"></span>
            <span class="size-2 animate-bounce rounded-full bg-cyan-300 [animation-delay:150ms]"></span>
            <span class="size-2 animate-bounce rounded-full bg-violet-300 [animation-delay:300ms]"></span>
            <span class="text-xs font-black uppercase tracking-widest text-slate-300">Procesando resultados en vivo</span>
          </div>
        </div>
      ...
      <section v-else class="mt-8 space-y-4">
        <article
          ...
          :key="contestant.id"
          class="rounded-3xl border border-white/10 bg-white/5 p-4 transition hover:bg-white/8 sm:p-5"
          :class="contestant.isWinner && 'border-amber-300/40 bg-amber-400/10'"
        >
          <div class="flex flex-col gap-4 md:flex-row md:items-center">
            <span class="text-3xl font-black text-fuchsia-200">#{{ index + 1 }}</span>
            <span class="grid size-24 shrink-0 place-items-center overflow-hidden rounded-3xl bg-linear-to-br from-violet-500 to-fuchsia-500 text-3xl font-black text-white">
              <img
            ...
            <div class="min-w-0 flex-1">
              <h3 class="truncate text-2xl font-black text-white">{{ contestant.artist?.name || 'Artista' }}</h3>
              <p class="mt-1 text-sm font-bold uppercase text-fuchsia-200">
              ...
              <div class="mt-4 h-3 overflow-hidden rounded-full bg-white/10">
                <div
                  ...
                  :style="{ width: percentFor(contestant.totalVotes) }"
                ></div>
              </div>
              <p class="mt-2 text-sm font-bold text-slate-300">
                {{ contestant.totalVotes }} votos · {{ percentFor(contestant.totalVotes) }}
              </p>
            </div>
            <button
              type="button"
              class="min-h-12 rounded-2xl bg-linear-to-r from-violet-500 to-fuchsia-500 px-6 text-sm font-black uppercase tracking-wide text-white shadow-lg shadow-fuchsia-950/30 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-50"
