...
onMounted(() => {
  unsubscribePolls = subscribeLivePollsCached(null, (pollRows) => {
    ...
    polls.value = pollRows.map((poll) => {
      ...
      const activeRoundId =
        poll.activeRoundId || previousPoll?.activeRoundId || "";
...
<template>
  <section class="active-polls-surface mx-auto max-w-352 py-6 px-4 sm:px-6 lg:py-8">
    ...
    >
      ...
      ></div>
      <div
        class="pointer-events-none absolute -bottom-24 right-0 size-72 rounded-full bg-cyan-400/10 blur-3xl"
      ></div>
      <div
        class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_78%_46%,rgba(217,70,239,0.16),transparent_24%)]"
      ></div>
      <div
        class="relative grid gap-5 md:grid-cols-[auto_1fr_auto] md:items-center"
      >
        <div
          class="grid size-18 place-items-center rounded-3xl border border-fuchsia-300/25 bg-fuchsia-400/10 text-3xl text-fuchsia-100 shadow-xl shadow-fuchsia-950/20"
        >
        ...
        <div>
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <span
              class="rounded-full border border-fuchsia-300/20 bg-fuchsia-400/10 px-3 py-1 text-xs font-black text-fuchsia-100"
            >
    ...
    <div
      v-else
      class="mobile-active-polls-slider -mx-4 flex snap-x gap-4 overflow-x-auto px-4 pb-2 md:hidden"
    >
      <article
